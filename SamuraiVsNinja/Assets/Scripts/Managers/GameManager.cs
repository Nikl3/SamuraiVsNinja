...
public class GameManager : SingeltonPersistant<GameManager>
{
    ...
    public GAME_STATE_OLD CurrentGameState { get; private set; }
    ...
    protected override void Awake()
    {
        ...
        screenFadeImage = UIManager_Old.Instance.transform.Find("FadeImage").GetComponent<Image>();
        loadImageObject = UIManager_Old.Instance.transform.Find("LoadImage").gameObject;
    ...
    private void CheckCurrentScene()
    {
        ...
        {
            case "MainMenuScene":
                ChangeGameState(GAME_STATE_OLD.MAIN_MENU);
                break;

            case "Game":
                ChangeGameState(GAME_STATE_OLD.LOCAL_GAME);
                break;

            case "Online":
                ChangeGameState(GAME_STATE_OLD.ONLINE_GAME);
                break;
        }
    }
    ...
    private void Clear()
    {
        ...
        foreach (var playerData in PlayerDataManager.Instance.PlayerData)
        {
    ...
    public void ChangeGameState(GAME_STATE_OLD newGameState)
    {
        CurrentGameState = newGameState;

        switch (CurrentGameState)
        {
            case GAME_STATE_OLD.MAIN_MENU:
                UIManager_Old.Instance.SetMainMenuUI();
                break;

            case GAME_STATE_OLD.LOCAL_GAME:
                UIManager_Old.Instance.SetLevelUI();
                break;

            case GAME_STATE_OLD.ONLINE_GAME:
                UIManager_Old.Instance.SetOnlineUI();
                break;
    ...
    public void LoadSceneAsync(int sceneIndex)
    {
        ...
        {
            if(CurrentGameState != GAME_STATE_OLD.MAIN_MENU)
            {
                Clear();
            }
    ...
    private IEnumerator ILoadSceneAsync(int sceneIndex)
    {
        ...
        loadImageObject.SetActive(CurrentGameState == GAME_STATE_OLD.MAIN_MENU ? true : false);
        ...
        {
            if (asyncOperation.progress == 0.9f && !UIManager_Old.Instance.IsLoadImageAnimationRunning)
            {
                if (!isAnimatingText)
                {
